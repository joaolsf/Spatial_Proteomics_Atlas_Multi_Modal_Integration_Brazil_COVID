...
#2- Load and scale data - Preprocessing
...
write.csv(mydata, "COVID_recovered_early_late_death_MOFA_MEFISTO_scaled.csv")

mydata3 <- read.csv("COVID_recovered_early_late_death_MOFA_MEFISTO_scaled_v2.csv")
library(reshape)
data.m <- melt(mydata3)
write.csv(data.m, "COVID_recovered_early_late_death_MOFA_MEFISTO3.csv")
...
mydata4 <- read.csv("COVID_early_late_death_MOFA_MEFISTO_iteration1.csv")
...
#3.2- Visualise data structure overview
...
outfile <- "COVID_early_late_death_model_iteration1_MEFISTO_day_of_symptoms.hdf5"

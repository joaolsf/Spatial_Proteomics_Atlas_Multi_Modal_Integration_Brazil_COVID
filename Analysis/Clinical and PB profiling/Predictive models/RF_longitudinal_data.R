...
mydata2 <- mydata[1:47,] #RF analysis within the fatal group
...
htree2 = hrf(x=mydata2, id=mydata2$RecordID, time=mydata2$Day, yindx="Outcome2", control=control) # model for the outcome recovered vs early vs late death
...
# 2- Variable Importance and Selection with VSURF package
...
# 2.2- Variable Importance and Selection with VSURF package - Recovered vs Early vs Late
...
# 2.3- Variable Importance and Selection with VSURF package - Early vs Late Death only 
...
# 3- Run random forest rmodel with the variables important for prediction as determined above - only data from days 0 and 3 and including clinical and luminex panel 1 data from patients d0
# 3.1- Recovered vs fatal
...
# 3.2- Recovered vs Early death vs Late death - only data from days 0 and 3
...
# 3.3- Early death vs Late death - only data from days 0 and 3

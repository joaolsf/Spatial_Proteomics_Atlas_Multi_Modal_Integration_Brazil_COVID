...
saveRDS(rf_model,"rftrain_model.rds")
rf_model <- readRDS("rftrain_model.rds")
...
write.csv(Important_Features, "Important_Features.csv")
Important_Features <- read.csv("Important_Features.csv")
...
write.csv(Gini_Features, "Gini_Features.csv")
Gini_Features <- read.csv("Gini_Features.csv")
...
predict_plot<-plot_predict_interaction(forest, bake(covid_prep, new_data = train_data), "ALT", "PD_L1", main = "Distribution of Predicted Probability of COVID-19 Progression") + 
...
test_plot<-ggplot(train_data, aes(ALT, PD_L1, colour=Outcome)) +geom_jitter(size=8) +
  labs(title="Distribution of COVID-19 Progression", y="PD-L1") + 
  theme_light(base_size = 16) +  
  scale_color_manual(values=c("grey", "salmon", "light blue")) +  
  theme(legend.position="bottom") +
...
predict_plot<-plot_predict_interaction(forest, bake(covid_prep, new_data = train_data), "IL33", "IL12p70", main = "Distribution of Predicted Probability of COVID-19 Progression") + 
  theme(legend.position="bottom") + 
  geom_hline(yintercept = 200, linetype="longdash") + 
  geom_vline(xintercept = 30, linetype="longdash")

test_plot<-ggplot(test_data, aes(IL33, IL12p70, colour=Outcome)) +geom_jitter(size=8) +
  labs(title="Distribution of COVID-19 Progression", y="IL12p70") + 
  theme_light(base_size = 16) +
  scale_color_manual(values=c("grey", "salmon", "light blue")) +  
  theme(legend.position="bottom") +
...
# 2.1- Recovered vs ED vs LD

# train model with random_forest
set.seed(1)
forest_final <- randomForest::randomForest(Outcome ~ IL12p70+IFNb+PD_L1+IFNlambda2+IL33+IL27+Lymphocytes+CCL1+Platelets+NCLR+ALT, data = bake(covid_prep, new_data = train_data), localImp = TRUE, importance=T, ntree = 1000, type= "classification")

saveRDS(forest_final,"forest_final_recovered_ED_LD.rds")
#forest_final <- readRDS("forest_final_recovered_ED_LD.rds")

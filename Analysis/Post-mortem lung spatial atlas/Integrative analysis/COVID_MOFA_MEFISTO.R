...
# 2.1- Load and scale data
# Data is stored as dataframe.
multiplex <- read.csv("MOFA_Brazil_Rendeiro_iteration1.csv")
...
df_to_cluster_scale <- df_to_cluster[,1:52]
df_to_cluster_supp <-df_to_cluster$Category
  
# Prepare data
mydata <- scale(df_to_cluster_scale)
mydata2 <- cbind.data.frame(mydata, df_to_cluster_supp)
write.csv(mydata, "MFA_Clinical_IMC_Complete_scaled.csv")


# 2.2- Remove batch effects (differences between cohorts and gender) and correct data by Age (covariate) with limma package
mydata5 <- read.csv("Brazil_Rendeiro_last_sample_only.csv")

# convert columns for correction to vectors
batch <- as.vector(mydata5$Batch)
gender <- as.vector(mydata5$Gender)
group <- as.vector(mydata5$Group)

# transpose dataframe to run correction -  samples as columns and features as rows
mydata6 <- mydata5[,7:211]
rownames(mydata6) <- mydata5$RecordID
mydata6 <- t(mydata6)

# convert columns for covariate to matrix - v1 = correct by age and average DOS; v2 = correct only by age
mydata7 <-mydata5[,5]
mydata7 <- as.matrix(mydata7)

# check dimensions
dim(mydata6)
dim(mydata7)
dim(matrix(1,ncol(mydata6),1))

# run the remove batch effect function and save the data
mydata8 <- removeBatchEffect(mydata6, batch = batch, batch2 = gender, design = matrix(1,ncol(mydata6),1),
                             covariates = mydata7, group = group)  
# transpose the output - samples as rows and features as columns
mydata9 <- (t(mydata8))
#design=model.matrix(~group), 
write.csv(mydata9, "Brazil_Rendeiro_last_sample_only_corrected.csv")

# scale data
mydata10 <- scale(mydata9)
write.csv(mydata10, "Brazil_Rendeiro_last_sample_only_corrected_scaled_v3.csv")
...
mydata <- read.csv("Brazil_Rendeiro_last_sample_only_corrected_scaled.csv")
library(reshape)
data.m <- melt(mydata)
write.csv(data.m, "MOFA_Brazil_Rendeiro_last_sample_only_corrected_scaled.csv")

mydata <- read.csv("MOFA_Brazil_Rendeiro_last_sample_only_corrected_scaled.csv")
...
# 3.2- Create MOFAobject
MOFAobject_untrained <- create_mofa(mydata) #if using MOFA2 - I am using MOFA (1) because I want to regress out the covariates...looks like this has been dropped in v2
MOFAobject_untrained
#groups = "group"

# 3.2.1- If using the time as covariate for MEFISTO analysis
...
#MOFAobject_untrained <- set_covariates(MOFAobject_untrained, covariates = "Batch")
#MOFAobject_untrained

#"Average_Day_of_Symptoms"

# 3.2.2- Visualise data structure overview
...
#3.3.4- MEFISTO options (if using time or space as covariate)
...
#3.5- Train the MOFA model 
#Now, we are ready to train the model.
outfile <- "MOFA_Brazil_Rendeiro_last_sample_only_v4.hdf5"
...
# Load the model
outfile2 <- "MOFA_Brazil_Rendeiro_last_sample_only.hdf5"
...
plot_factor(MOFAobject,
            ...
            color_by = "Group", 
            scale = FALSE, 
            add_violin = FALSE, color_violin = TRUE, add_boxplot = T,boxplot_alpha = 0.7,
            dodge = TRUE, dot_size = 2, legend = TRUE
) + scale_color_manual(values=group.colors) + scale_fill_manual(values=group.colors) + theme_bw(base_size=20) +
  theme(plot.title = element_text(size=20), 
        strip.text.x = element_text(size = 20, colour = "black", face = "plain"),
        strip.background = element_rect(color=NULL, fill="#E7B800", linewidth = 1.5, linetype="solid"))

multiplex <- read.csv("./Last sample/Factor_values_v4.csv")
multiplex$title <- "Integrated signature 5"
p <- ggplot(multiplex, aes(x=Category, y=Factor5, fill=Category)) +
  geom_boxplot(outlier.alpha = 0.1, outlier.shape = 2)  + 
  geom_jitter(size = 3, shape=16, position=position_jitter(0.025)) +
  ...
  theme(legend.justification=c(),
        ...
        strip.text.x = element_text(size = 18, colour = "black", face = "plain"),
        strip.background = element_rect(color=NULL, fill="#E7B800", size=1.5, linetype="solid")) +
  geom_hline(yintercept=0,linetype=2)

p + scale_fill_manual(values = c( "salmon", "light blue")) + scale_x_discrete(limits=c("Early death", "Late death"))  + facet_grid(. ~ title) + scale_y_continuous(limit = c(-3, 3))
...
plot_factors_vs_cov(MOFAobject, color_by = "Group", factor = 1) +
  scale_fill_manual(values = group.colors) +
  stat_summary(aes(col = color_by), geom="line", fun = "mean", size=2) +
...
plot_weights_fn <- function(MOFAobject, factor=5, view=1, nfeatures=10) {
...
plot_weights_fn(MOFAobject, factor=1, view="Luminex", nfeatures=20) 
plot_weights_fn(MOFAobject, factor=1, view="Clinical", nfeatures=10)
plot_weights_fn(MOFAobject, factor=1, view="IMC", nfeatures=60) 
...
weights <- read.csv("./Last sample/Factor1_IMC_features_weights_v4_edited.csv")
plot_ <- ggplot(weights,
  ...
  xlab("") + 
  ylab("Factor weight") +
  ggtitle("Tissue parameters - Integrated signature 1") +
  theme(plot.title = element_text(size=16), 
        axis.text.y = element_text(face = "plain", size=10, angle=0, vjust=.5, hjust=1),
        axis.text.x = element_text(face = "plain", size=16, angle=0, vjust=.5, hjust=1)) 
plot_
...
ggplot(df, aes(x=factor, y=coef, ymin=lower, ymax=higher)) +
  ...
  theme_bw(base_size = 16) + scale_color_manual(values = c("black", "red"))
...
write.csv(factors3,'Factor_values_v4.csv')
...
weights <- get_weights(MOFAobject, as.data.frame = T, factors = 1, views = 'IMC')
head(weights, n=3)
write.csv(weights,'Factor1_IMC_features_weights_v4.csv')
